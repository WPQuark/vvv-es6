#!/usr/bin/env bash
# Provision a simple Nginx server with PHP support
...
# Install the PHP directory indexer
if [[ ! -f "${VVV_PATH_TO_SITE}/public_html/index.php" ]]; then
	mkdir -p "${VVV_PATH_TO_SITE}/public_html" 2> /dev/null
	cp -f "${VVV_PATH_TO_SITE}/provision/index.php" "${VVV_PATH_TO_SITE}/public_html/index.php"
	sed -i "s#{{SITE_TITLE_HERE}}#${SITE_TITLE}#" "${VVV_PATH_TO_SITE}/public_html/index.php"
fi
